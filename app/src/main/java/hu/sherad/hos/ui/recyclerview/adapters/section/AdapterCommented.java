...
public class AdapterCommented extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements ItemTouchHelperCallback.ItemTouchHelperAdapter {
    ...
    public AdapterCommented(SortableModifiableTopicActions actions) {
    ...
    public int getItemViewType(int position) {
        if (position == UserTopics.getInstance().getTopics(Topic.Type.COMMENTED).size()) {
    ...
    public int getItemCount() {
        return UserTopics.getInstance().getTopics(Topic.Type.COMMENTED).size() + (UserTopics.getInstance().getData() == PH.Data.OK ? 0 : 1);
    }

    public void setTopicToSeen(Topic topicToSeen) {
        int position = UserTopics.getInstance().setTopicToSeen(Topic.Type.COMMENTED, topicToSeen);
        if (position == RecyclerView.NO_POSITION) {
            Logger.getLogger().i("No topic found with [" + topicToSeen.getTitle() + "] title");
        } else {
            notifyItemChanged(position);
        }
        // Also if it's in the favourites section update it
        UserTopics.getInstance().setTopicToSeen(Topic.Type.FAVOURITE, topicToSeen);
    }

    public void deleteTopic(Topic topicToDelete) {
        int position = UserTopics.getInstance().removeTopic(Topic.Type.COMMENTED, topicToDelete);
        if (position == RecyclerView.NO_POSITION) {
    ...
    public void reloadData() {
        Logger.getLogger().i(
                "Reloading topics: " + getItemCount() + " to " + UserTopics.getInstance().getTopics(Topic.Type.COMMENTED).size());
    ...
    private void bindViewAsTopic(HolderListTopic holder, int position) {
        Topic topic = UserTopics.getInstance().getTopics(Topic.Type.COMMENTED).get(position);
        // Title
        ...
        if (isExpanded) {
            ...
            // Notification
            Set<String> savedTopics = NotificationUtils.getNotifiableTopics(Topic.Type.COMMENTED);
            holder.getImageButtonNotification().setSelected(savedTopics.contains(topic.getTitle()));
            holder.getImageButtonNotification().setOnClickListener(view -> NotificationUtils.toggleNotificationWithDialog(topic, Topic.Type.COMMENTED, holder));
            // EXTRA_URL copy
    ...
    private void bindViewAsText(HolderListSimpleView holder, int position) {
        ...
        if (position == UserTopics.getInstance().getTopics(Topic.Type.COMMENTED).size() && UserTopics.getInstance().getData() == PH.Data.OK) {
    ...
    public void onItemMove(int fromPosition, int toPosition) {
        if (fromPosition < toPosition) {
            for (int i = fromPosition; i < toPosition; i++) {
                Collections.swap(UserTopics.getInstance().getTopics(Topic.Type.COMMENTED), i, i + 1);
            }
        } else {
            for (int i = fromPosition; i > toPosition; i--) {
                Collections.swap(UserTopics.getInstance().getTopics(Topic.Type.COMMENTED), i, i - 1);
